...
Porém, esta definiçao não engloba todas as possibilidade. Considere dois sensores GPS localizados em posições diferentes. É possível a recuperação da localização e classifica-la como sendo um contexto primário de cada sensor, porém, a distância apresentada entre as duas localizações só pode ser adquirida através de calculos dos dados encontrados. A questao é qual é a categoria que distância pertence? primária, ou secundária? A distância não é um valor percebido pelo meio. Ao contrário, é um valor computado a partir de dois contextos. A categorizaçao proposta acima seria insuficiente para representação precisa deste caso.
